 ...
 *
 * Ingest a DC monthly-solicitation page into Supabase.
 *
 *   npx tsx scripts/ingest-dc.ts <url>
 *
 * Pipeline:
 *   1. Fetch the solicit page HTML
 *   2. Strip it to the main article text
 *   3. Send it to Claude (claude-sonnet-4-6) to extract every solicited item
 *      as a strict JSON array (chunked + merged if the page is long)
 *   4. Validate every row; invalid rows go to a `rejects` list (never the DB)
 *   5. Upsert valid rows into Supabase (service-role key), creating/linking
 *      series and creators, and record a row in `ingest_runs`
 *   6. Print a summary
 *
 ...
 *
 * This script writes to the catalog + ops tables, which have RLS enabled. It
 * uses the service-role/secret key, which bypasses RLS — do NOT run it with a
...
const PUBLISHER_SLUG = "dc";
const DEFAULT_URL =
  "https://www.comicreleases.com/2026/04/dc-july-2026-solicitations/";
const CHUNK_CHARS = 40_000; // ~10-12k tokens of article text per extraction call
const SOURCE = "ingest:solicits:dc";
...
const EXTRACTION_PROMPT = `You are a data-extraction engine for comic-book solicitations.

You will be given the text of a monthly DC solicitations page. Extract EVERY
solicited product into a JSON array. One object per product; treat each distinct
variant cover as its own object.

Field rules:
- series_name: the ongoing/mini series or collection title WITHOUT the issue
  number (e.g. "Absolute Batman", "Detective Comics").
...
async function main(): Promise<void> {
  ...
  if (!process.argv[2]) {
    console.log(`No URL argument given — using default:\n  ${url}\n`);
  }
  ...
  // Resolve the DC publisher (seeded by the migration).
  const { data: publisher, error: pubErr } = await db
    .from("publishers")
    .select("id")
    .eq("slug", PUBLISHER_SLUG)
    .maybeSingle();
  if (pubErr) {
    throw new Error(
  ...
  if (!publisher) {
    throw new Error(
      `Publisher '${PUBLISHER_SLUG}' not found. Apply the schema migration (which seeds publishers) before ingesting.`,
    );
  }
  ...
  try {
    ...
    console.log(`Extracting via ${MODEL} across ${chunks.length} chunk(s)...`);
    const all: ExtractedItem[] = [];
    for (let i = 0; i < chunks.length; i++) {
      const label = `chunk ${i + 1}/${chunks.length}`;
      const items = await extractChunk(anthropic, chunks[i], label);
    ...
    console.log(
      `Extracted ${all.length} raw, ${extractedCount} after de-dup.`,
    );
    ...
    // 5. Upsert.
    const ingestor = new Ingestor(db, publisher.id, url);
    for (const item of valid) {
      try {
        ...
        rejects.push({
          item,
          reasons: [`db error: ${(e as Error).message}`],
        });
      }
    }
